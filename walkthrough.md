...
*   **Habilitação de Gestos Verticais (touch-action)**: Além de `pointer-events: auto !important`, declaramos explicitamente `touch-action: pan-y !important;` tanto na classe contêiner `.webtoon-canvas` quanto nas imagens `.webtoon-page-img` no bloco mobile, sinalizando ao motor de renderização móvel que a tela aceita e aguarda arrastos de navegação vertical.
*   **Garantia de Topo no Carregamento Tardio**: Em [js/ler.js](file:///c:/Users/Barbara/Desktop/miles/site-fiovermelho/js/ler.js), se a primeira página carregada for a inicial (`i === 1`), força-se imediatamente a rolagem instantânea para o topo (`scrollTo(0,0)`) envelopado em um `setTimeout` de 100ms e com múltiplos fallbacks de propriedade (`documentElement.scrollTop` e `body.scrollTop`) para dar tempo de o navegador calcular com exatidão o carregamento da fita.
